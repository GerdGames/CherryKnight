...
void UGBAAttributeSetBlueprint::OnVariableAdded(const FName& Name) const
{
	GBA_NS_LOG(Verbose, TEXT("Name: %s"), *Name.ToString())
	const UPackage* Package = GetPackage();
	if (!Package)
	{
		GBA_NS_LOG(Warning, TEXT("SelfPackage nullptr"))
		return;
	}
	
	FGBADelegates::OnVariableAdded.Broadcast(Package->GetFName(), Name);
}

void UGBAAttributeSetBlueprint::OnVariableRemoved(const FName& Name) const
{
	GBA_NS_LOG(Verbose, TEXT("Name: %s"), *Name.ToString())
	...
	FGBADelegates::OnVariableRemoved.Broadcast(Package->GetFName(), Name);
}
