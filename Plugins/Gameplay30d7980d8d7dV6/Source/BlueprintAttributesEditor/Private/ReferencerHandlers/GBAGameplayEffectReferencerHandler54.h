 ...
 *
 * Important: This class is the "legacy" version for 5.4 and below. 5.5 introduces some changes in the way FGameplayAttribute
 * gets serialized that needs special handling that is done in `FGBAGameplayEffectReferencerHandler`, this file remains
 * here and used for 5.4 and below to not impact previous engine versions where renamed or removed attribute were working.
 *
 * Renamed via what this referencer handler does (e.g. updating GEs that were referencing it to point to new attribute name).
 * Removed was okay (as in, no editor crash) where the Attribute was appearing as None and didn't need special handling.
 *
 ...
 *
 * The new version active in 5.5 also avoids th we use of FGameplayAttribute::StaticStruct()->ExportText, which can also
 * be problematic.
 *
 * The PackageName and AttributeName info can be determined out of the Property PathName `Property->GetPathName()`.
 */
class FGBAGameplayEffectReferencerHandler54 : public IGBAAttributeReferencerHandler
{
public:
	...
	virtual ~FGBAGameplayEffectReferencerHandler54() override;
	...
	virtual bool HandleAttributeRemoved(const FAssetIdentifier& InAssetIdentifier, const FGBAAttributeReferencerPayload& InPayload, TArray<TSharedRef<FTokenizedMessage>>& OutMessages) override;
