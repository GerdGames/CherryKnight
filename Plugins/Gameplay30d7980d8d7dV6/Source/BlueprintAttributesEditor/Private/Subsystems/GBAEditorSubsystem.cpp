...
#include "ReferencerHandlers/GBACoreRedirectReferencerHandler.h"
#include "ReferencerHandlers/GBAGameplayEffectReferencerHandler.h"
#else
#include "ReferencerHandlers/GBAGameplayEffectReferencerHandler54.h"
#endif
...
void UGBAEditorSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	FGBADelegates::OnVariableRemoved.AddUObject(this, &UGBAEditorSubsystem::HandleAttributeRemoved);
	...
	// Register handlers
#if UE_VERSION_NEWER_THAN(5, 5, -1)
	// 5.5 and beyond uses the new reference handler (which is the same class name FGBAGameplayEffectReferencerHandler)
	// 5.5 needs extra handling for removed variables, and attribute rename needs to account for Attribute.IsValid() being true
	// in 5.5 (even if underlying attribute property is not, and do not have a proper owner anymore).
	
	RegisterReferencerHandler(TEXT("GameplayEffect"), FGBAGameplayEffectReferencerHandler::Create());

	// New one, using core redirects
	// RegisterGlobalReferencerHandler(TEXT("GameplayAttributeCoreRedirects"), FGBACoreRedirectReferencerHandler::Create());
#else
	// 5.4 and below uses the old implementation, which was working up until 5.4
	RegisterReferencerHandler(TEXT("GameplayEffect"), FGBAGameplayEffectReferencerHandler54::Create());
#endif
}
...
void UGBAEditorSubsystem::Deinitialize()
{
	...
	RegisteredGlobalHandlers.Reset();
...
void UGBAEditorSubsystem::RegisterGlobalReferencerHandler(const FName& InClassName, const TSharedPtr<IGBAAttributeGlobalHandler>& InHandler)
{
	RegisteredGlobalHandlers.Emplace(InClassName, InHandler);
}

void UGBAEditorSubsystem::UnregisterGlobalReferencerHandler(const FName& InClassName)
{
	if (RegisteredGlobalHandlers.Contains(InClassName))
	{
		RegisteredGlobalHandlers.Remove(InClassName);
	}
}
...
void UGBAEditorSubsystem::HandleAttributeRename(const FName& InPackageName, const FName& InOldPropertyName, const FName& InNewPropertyName)
{
	...
	// Check if we have global (catch all) registered handler
	TArray<TSharedRef<FTokenizedMessage>> Messages;
	for (const TPair<FName, TSharedPtr<IGBAAttributeGlobalHandler>>& GlobalHandler : RegisteredGlobalHandlers)
	{
		TSharedPtr<IGBAAttributeGlobalHandler> Handler = GlobalHandler.Value;
		if (Handler.IsValid())
		{
			...
			// Payload.Referencers = Referencers;
			// Payload.ReferencerAssets = ReferencerAssets;
			
			Handler->HandleAttributeRename(ReferencerAssets, Payload, Messages);
	...
	UpdateReferencersForRename(ReferencerAssets, InPackageName, InOldPropertyName, InNewPropertyName);
	...
	UpdateK2NodeReferencersForRename(InPackageName, InOldPropertyName, InNewPropertyName);
	...
	if (!PendingMessages.IsEmpty())
	{
		...
		const FText NotifyText = FText::Format(LOCTEXT("NotifyText", "Blueprint Attributes: {0} ({1} updates)\n\nDon't forget to save the assets that were changed."), PageText, FText::AsNumber(PendingMessages.Num()));
...
void UGBAEditorSubsystem::HandleAttributeRemoved(const FName& InPackageName, const FName& InPropertyName)
{
	GBA_EDITOR_NS_LOG(Verbose, TEXT("InPackageName: %s, InName: %s"), *InPackageName.ToString(), *InPropertyName.ToString())

	if (bIsHandlingRename)
	{
		GBA_EDITOR_NS_LOG(Verbose, TEXT("Handling rename already. InPackageName: %s, InPropertyName: %s"), *InPackageName.ToString(), *InPropertyName.ToString())
		return;
	}
	...
	// Keep a copy of old name to display with the notif and message log
	const FName RemovedPropertyName = InPropertyName;
	
	FScopedSlowTask Progress(3.f, LOCTEXT("SlowTask_UpdateReferencers_Removed", "Removed attribute - Gather referencers"));
	Progress.MakeDialog();
	...
	// Handle Gameplay Effect Referencers ...
	Progress.EnterProgressFrame(1.f, FText::Format(LOCTEXT("SlowTask_UpdateReferencers_Removed", "Removed attribute - Update {0} referencers"), FText::AsNumber(Referencers.Num())));
	...
	for (const TPair<FName, TSharedPtr<IGBAAttributeGlobalHandler>>& GlobalHandler : RegisteredGlobalHandlers)
	{
		...
		if (Handler.IsValid())
		{
			FGBAAttributeReferencerPayload Payload;
			Payload.PackageName = InPackageName.ToString();
			Payload.RemovedPropertyName = InPropertyName.ToString();
			
			Handler->HandleAttributeRemoved(ReferencerAssets, Payload, Messages);
			PendingMessages.Append(Messages);
		}
	}
	
	// Now, update any referencers with FGameplayAttribute properties to point to the new one, from the previously renamed prop
	UpdateReferencersForRemoval(ReferencerAssets, InPackageName, InPropertyName);

	Progress.EnterProgressFrame(1.f, LOCTEXT("SlowTask_UpdateK2Referencers_Removed", "Removed attribute - Update K2 Nodes referencers"));
	
	// Handle K2Node Referencers ...
	UpdateK2NodeReferencersForRemoval(InPackageName, InPropertyName);
	...
	if (!PendingMessages.IsEmpty())
	{
		...
		FMessageLog MessageLog(LogName);
		const FText PageText = FText::Format(LOCTEXT("PageInfo", "Removed attribute - Updated from {0} to {1}"), FText::FromName(RemovedPropertyName), FText::FromName(PropertyName));
		MessageLog.NewPage(FText::Format(LOCTEXT("NewPage", "{0} ({1})"), PageText, FText::AsDateTime(FDateTime::Now())));
...
void UGBAEditorSubsystem::HandlePreCompile(const FName& InPackageName)
{
	...
	// Invoke OnPreCompile on handlers, often used to clean up before handling pre-compile
	for (const TPair<FName, TSharedPtr<IGBAAttributeGlobalHandler>>& GlobalHandler : RegisteredGlobalHandlers)
	{
		TSharedPtr<IGBAAttributeGlobalHandler> Handler = GlobalHandler.Value;
		if (Handler.IsValid())
	...
	// Handle K2 Nodes (GetFloatAttribute, GetAttributeValue, etc. - nodes with FGameplayAttribute pin parameters)
	for (TObjectIterator<UK2Node> NodeIt; NodeIt; ++NodeIt)
	{
		...
		// If we have a handler for this K2 Node, delegate any custom handling of an attribute rename (for instance, custom K2 Node switch with array property of FGameplayAttribute)
...
void UGBAEditorSubsystem::UpdateK2NodeReferencersForRename(const FName& InPackageName, const FName& InOldPropertyName, const FName& InNewPropertyName)
{
...
void UGBAEditorSubsystem::UpdateK2NodeReferencersForRemoval(const FName& InPackageName, const FName& InRemovedPropertyName)
{
	GBA_EDITOR_NS_LOG(Verbose, TEXT("InPackageName: %s, InOldPropertyName: %s"), *InPackageName.ToString(), *InRemovedPropertyName.ToString())
	
	UObject* OwnerAttributeSet = LoadObject<UObject>(nullptr, *InPackageName.ToString());
	if (!OwnerAttributeSet)
	{
		GBA_EDITOR_NS_LOG(Warning, TEXT("Cannot load AttributeSet class from %s"), *InPackageName.ToString())
		return;
	}
	...
	if (!OwnerAttributeSetBP)
	{
		GBA_EDITOR_NS_LOG(Warning, TEXT("Cannot load AttributeSet class Blueprint from %s"), *InPackageName.ToString())
		return;
	}

	// Gather the list of Blueprints with K2Node using a Pin Attribute parameter
	const TArray<FPinToModify> PinsToModify = GetPinsToReset(InPackageName.ToString(), InRemovedPropertyName.ToString());
	
	// Actually update the list of pins to point to the new attribute property
	if (!PinsToModify.IsEmpty())
	{
		HandlePinsToReset(PinsToModify, OwnerAttributeSetBP, InRemovedPropertyName);
	}
}
...
TArray<UGBAEditorSubsystem::FPinToModify> UGBAEditorSubsystem::GetPinsToReset(const FString& InPackageName, const FString& InRemovedPropertyName)
{
	TArray<FPinToModify> PinsToReset;
	...
	for (TObjectIterator<UK2Node> NodeIt; NodeIt; ++NodeIt)
	{
		...
		// If we have a handler for this K2 Node, delegate any custom handling of an attribute removal (for instance, custom K2 Node switch with array property of FGameplayAttribute)
		UClass* NodeClass = Node->GetClass();
		if (NodeClass && RegisteredHandlers.Contains(NodeClass->GetFName()))
		{
			TSharedPtr<IGBAAttributeReferencerHandler> Handler = RegisteredHandlers.FindChecked(NodeClass->GetFName());
			if (Handler.IsValid())
			{
				...
				Payload.DefaultObject = Node;
				Payload.PackageName = InPackageName;
				Payload.RemovedPropertyName = InRemovedPropertyName;
				
				TArray<TSharedRef<FTokenizedMessage>> Messages;
				FAssetIdentifier AssetIdentifier(Node, NAME_None);
				Handler->HandleAttributeRemoved(AssetIdentifier, Payload, Messages);
				PendingMessages.Append(Messages);
			}
		}
		...
		for (UEdGraphPin* Pin : Node->Pins)
		{
			...
			if (Pin->Direction == EGPD_Input)
			{
				...
				if (PackageName == InPackageName && AttributeName == InRemovedPropertyName)
				{
					PinsToReset.AddUnique(FPinToModify(Pin, Node, AttributeName));
				}
			}
		}
	}

	return MoveTemp(PinsToReset);
}
...
void UGBAEditorSubsystem::HandlePinsToReset(const TArray<FPinToModify>& InPinsToModify, const UBlueprint* InOwnerAttributeSetBP, const FName& InRemovedPropertyName)
{
	TSet<UBlueprint*> BlueprintsModified;
	...
	for (const FPinToModify& PinToModify : InPinsToModify)
	{
		...
		if (!InOwnerAttributeSetBP->GeneratedClass)
		{
			GBA_EDITOR_NS_LOG(Warning, TEXT("NO GENERATED CLASS (InOwnerAttributeSetBP: %s"), *GetNameSafe(InOwnerAttributeSetBP))
			continue;
		}
...

		{
			...
			FGameplayAttribute::StaticStruct()->ExportText(FinalValue, &DummyAttribute, &DummyAttribute, nullptr, PPF_SerializedAsImportText, nullptr);
			...
			if (FinalValue != DefaultValue && Pin->GetSchema())
			{
				...
				FText MessageText = FText::Format(
					LOCTEXT("ChangedPinFromTo", "from {0} to {1}"),
					FText::FromString(PinToModify.OldPropertyName),
					FText::FromString(DummyAttribute.GetName())
				);
				Message->AddToken(FTextToken::Create(MessageText));
	...
	GBA_EDITOR_NS_LOG(Verbose, TEXT("BlueprintsModified: %d"), BlueprintsModified.Num())
	for (UBlueprint* Blueprint : BlueprintsModified)
	{
		if (!Blueprint)
		{
		...
		GBA_EDITOR_NS_LOG(Verbose, TEXT("\t MarkBlueprintAsStructurallyModified Blueprint: %s"), *Blueprint->GetName())
		FBlueprintEditorUtils::MarkBlueprintAsStructurallyModified(Blueprint);

		FString PathName = Blueprint->GetPathName();
		GBA_EDITOR_NS_LOG(Verbose, TEXT("\t PathName: %s"), *PathName)
		...
		if (Blueprint == InOwnerAttributeSetBP)
		{
			GBA_EDITOR_NS_LOG(Verbose, TEXT("\t Blueprint: %s is the same. Needs compile again."), *Blueprint->GetName())
			bIsHandlingRename = true;
			FKismetEditorUtilities::CompileBlueprint(Blueprint);
			bIsHandlingRename = false;
			...
			if (Blueprint->Status == BS_UpToDate)
			{
				GBA_EDITOR_NS_LOG(Display, TEXT("Blueprint compiled successfully (%s)"), *Blueprint->GetName());
			}
			else if (Blueprint->Status == BS_UpToDateWithWarnings)
			{
				GBA_EDITOR_NS_LOG(Warning, TEXT("Blueprint compiled successfully with warnings(%s)"), *Blueprint->GetName());
			}
			else if (Blueprint->Status == BS_Error)
			{
				GBA_EDITOR_NS_LOG(Error, TEXT("Blueprint failed to compile (%s)"), *Blueprint->GetName());
			}
			else
			{
				GBA_EDITOR_NS_LOG(Error, TEXT("Blueprint is in an unexpected state after compiling (%s)"), *Blueprint->GetName());
			}
		}
	}
}
...
bool UGBAEditorSubsystem::ParseAttributeFromDefaultValue(const FString& InDefaultValue, FString& OutPackageName, FString& OutAttributeName)
{
	...
	// (AttributeName="Ref_01:z",Attribute=/Game/RenameTests/GBA_Ref_Test.GBA_Ref_Test_C:Ref_01:,AttributeOwner=BlueprintGeneratedClass'"/Game/RenameTests/GBA_Ref_Test.GBA_Ref_Test_C"')
...
void UGBAEditorSubsystem::UpdateReferencersForRename(TArray<FAssetData> InReferencers, const FName& InPackageName, const FName& InOldPropertyName, const FName& InNewPropertyName)
{
	...
	for (const FAssetData& Referencer : InReferencers)
	{
		...
		// Then check if we have a registered handler for this specific asset type
...
void UGBAEditorSubsystem::UpdateReferencersForRemoval(TArray<FAssetData> InReferencers, const FName& InPackageName, const FName& InRemovedPropertyName)
{
	GBA_EDITOR_NS_LOG(Verbose, TEXT("Referencer Assets: %d"), InReferencers.Num())

	FScopedSlowTask Progress(InReferencers.Num(), FText::Format(LOCTEXT("SlowTask_UpdateReferencersForRemoval", "Removed attribute - Update {0} referencers"), FText::AsNumber(InReferencers.Num())));
	Progress.MakeDialog();
	...
	for (const FAssetData& Referencer : InReferencers)
	{
		...
		FGBAAttributeReferencerPayload Payload;
		Payload.PackageName = InPackageName.ToString();
		Payload.RemovedPropertyName = InRemovedPropertyName.ToString();
		Payload.ReferencerBlueprint = Cast<UBlueprint>(Referencer.GetAsset());
		...
		if (Handler.IsValid() && Payload.DefaultObject.IsValid())
		{
			const bool bHandledRemoval = Handler->HandleAttributeRemoved(Referencer.PackageName, Payload, Messages);
			PendingMessages.Append(Messages);
