...
class IGBAAttributeGlobalHandler;
...
public:
	...
	/** List of referencer handler registered via RegisterReferencerHandler(), specific to a ClassName for the referencer */
	TMap<FName, TSharedPtr<IGBAAttributeReferencerHandler>> RegisteredHandlers;
	
	/**
	 * List of referencer handler registered via RegisterGlobalReferencerHandler(), those gets run once per event, instead of once per referencer
	 *
	 * The key FName in this case is just an identifier for the handler itself, instead of mapping the class name of the referencer being handled.
	 */
	TMap<FName, TSharedPtr<IGBAAttributeGlobalHandler>> RegisteredGlobalHandlers;
	...
	virtual void RegisterGlobalReferencerHandler(const FName& InClassName, const TSharedPtr<IGBAAttributeGlobalHandler>& InHandler);
	
	/** Removes a previously registered handler */
	virtual void UnregisterReferencerHandler(const FName& InClassName);
	virtual void UnregisterGlobalReferencerHandler(const FName& InClassName);

	/** Returns the associated referencer handler (if any) registered for the class default object of InReferencer.AssetId */
	TSharedPtr<IGBAAttributeReferencerHandler> FindAssetDependencyHandler(const FName& InPackageName, TWeakObjectPtr<UObject>& OutDefaultObject);
	...
	void HandleAttributeRemoved(const FName& InPackageName, const FName& InPropertyName);
	...
	void UpdateK2NodeReferencersForRename(const FName& InPackageName, const FName& InOldPropertyName, const FName& InNewPropertyName);
	void UpdateK2NodeReferencersForRemoval(const FName& InPackageName, const FName& InRemovedPropertyName);
	...
	TArray<FPinToModify> GetPinsToReset(const FString& InPackageName, const FString& InRemovedPropertyName);
	...
	void HandlePinsToReset(const TArray<FPinToModify>& InPinsToModify, const UBlueprint* InOwnerAttributeSetBP, const FName& InRemovedPropertyName);
	...
	void UpdateReferencersForRename(TArray<FAssetData> InReferencers, const FName& InPackageName, const FName& InOldPropertyName, const FName& InNewPropertyName);
	
	void UpdateReferencersForRemoval(TArray<FAssetData> InReferencers, const FName& InPackageName, const FName& InRemovedPropertyName);
